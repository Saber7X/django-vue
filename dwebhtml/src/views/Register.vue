    ...
    <div class="dweb loginbox">
      ...
      <el-form :label-position="'right'" label-width="70px" :model="formData">
        ...
        <el-form-item>
          ...
          <el-button @click="toLogin()" type="warning" plain>已有帐号</el-button>
        </el-form-item>
      </el-form>
    </div>
  </div>
...
export default {
  ...
  methods: {
    blogRegister() {
      ...
      if (this.formData.password.length !== this.formData.password2.length) {
        alert('表单填写完整')
        return
      }
      if (this.formData.length < 8) {
        alert('密码太短')
        return
      }

      //  提交注册
      this.$strore.display('blogRegister', this.formData)
    },
    toLogin() {
